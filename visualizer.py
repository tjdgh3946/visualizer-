...
class Visualizer:
	...
	def plot(self, marker=False, linestyle=None, grid=False, xlabel=None, ylabel=None, title=None):
	...
	def two_yscale_plot(self, marker=None, linestyle=None, grid=False, xlabel=None, ylabel=None, title=None): 
		...
		with plt.style.context(asahikawa['plot']):
			...
			plot1 = ax1.plot(self.x0, self.yy[0], label=self.labels[0], marker=marker, linestyle=linestyle)
			plot2 = ax2.plot(self.x0, self.yy[1], label=self.labels[1], marker=marker, linestyle=linestyle)
			if CI_flag:
				ax1.fill_between(self.x0, self.yy[0]-self.CI[0], self.yy[0]+self.CI[0], alpha=0.2)
				ax2.fill_between(self.x0, self.yy[1]-self.CI[1], self.yy[1]+self.CI[1], alpha=0.2)
	...
	def histogram(self, density=False, xlabel=None, ylabel=None, title=None):
		...
		with plt.style.context(asahikawa['hist']):
			for y, label in zip(self.yy, self.labels):
				plt.hist(y, alpha=.7, density=density, label=label)
			if xlabel:
				plt.xlabel(xlabel)
			if ylabel:
				plt.ylabel(ylabel)
	...
	def mutiple_bar(self, rotation=0, grid=False, show_value=False, ylabel=None, title=None):
		...
		with plt.style.context(asahikawa['plot']):
			...
			for i, (y, label) in enumerate(zip(self.yy, self.labels)):
				...
				plt.bar(offsets+width*i, y, width, yerr=yerr, label=label, ecolor='black', capsize=4)
				if show_value:
					ylim = plt.gca().get_ylim()[1]
					addlabels(offsets+width*i, y, label_offset=ylim*0.02)
			centering = width * (len(self.labels)/2) if len(self.labels)/2 ==0  else width*(len(self.labels)/2 -0.5)
	...
	def _csv_cleansing(self, path, axis):
		...
		data[data.select_dtypes("object").columns] = data[data.select_dtypes("object").columns].apply(lambda x : x.str.replace(',', ''))
